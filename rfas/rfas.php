...
if (isset($_GET['engineer'])) {
  $varEmployee_rsMaintenanceNotifs = (get_magic_quotes_gpc()) ? $_GET['engineer'] : addslashes($_GET['engineer']);
}
$varStatus_rsMaintenanceNotifs = "1";
if (isset($_GET['status'])) {
  $varStatus_rsMaintenanceNotifs = (get_magic_quotes_gpc()) ? $_GET['status'] : addslashes($_GET['status']);
}
$varApp_rsMaintenanceNotifs = "1";
if (isset($_GET['app'])) {
  $varApp_rsMaintenanceNotifs = (get_magic_quotes_gpc()) ? $_GET['app'] : addslashes($_GET['app']);
}
$varsummary_rsMaintenanceNotifs = "1";
if (isset($_GET['summary'])) {
  $varsummary_rsMaintenanceNotifs = (get_magic_quotes_gpc()) ? $_GET['summary'] : addslashes($_GET['summary']);
}
...
$rsChangeRequests = mysql_query($query_limit_rsChangeRequests, $connProdOps) or die(mysql_error());
...
<?php buildHeader("rfa", null, "rfas", "RFCs", "Add an RFC"); ?>
    <!-- TABS -->
    <div id="tabs">
        <span class="<?php if ((isset($_GET['app'])) || (isset($_GET['engineer'])) || (isset($_GET['requestOrigin'])) || (isset($_GET['status'])) || (isset($_GET['subapp'])) || (isset($_GET['summary']))) { echo "tabbak"; } else { echo "tabfor"; } ?>" id="tab_none"><a href="rfas.php?filter=none">No Filter</a></span>
        <?php
        tab("app","Application");
        ...
        <div id="tabscontent_none"<?php if ((isset($_GET['app'])) || (isset($_GET['engineer'])) ||(isset($_GET['requestOrigin'])) || (isset($_GET['status'])) || (isset($_GET['summary']))) { echo " style=\"display: none;\""; } ?>>Select a tab to filter the available RFCs</div>
        ...
        <div id="tabscontent_origin"<?php if (!isset($_GET['origin'])) { echo " style=\"display: none;\""; } ?>>
          <form action="rfas.php" method="get" name="filterOrigin" id="filterOrigin">
            Display RFCs for <select name="origin" id="origin">
                                <option value="">Select Origin</option>
                                <option value="Ticket"<?php if (isset($_GET['origin']) && ($row_rsRFA['requestOrigin'] == $_GET['engineer'])) { echo " selected=\"selected\"";} ?>><?php echo $row_rsEngineers['displayName']?></option>
            </select>&nbsp;#<input type="text" name="ticket" id="ticket"<?php if (isset($_GET['ticket'])) { echo " value=\"" . $_GET['ticket'] . "\""; } ?> size="10" />
            <input type="submit" name="Submit" value="Submit" />
          </form>
        </div>
        ...
        <div id="tabscontent_engineer"<?php if (!isset($_GET['employee'])) {
            ...
            <form action="rfas.php" method="get" name="filterEngineer" id="filterEngineer">
                Display RFCs submitted by <select name="engineer" id="engineer">
                    ...
                    <?php do { ?>
                        <option value="<?php echo $row_rsEngineers['employeeID'] ?>"<?php if (isset($_GET['engineer']) && ($row_rsEngineers['employeeID'] == $_GET['engineer'])) {
                        echo " selected=\"selected\"";
        ...
        <div id="tabscontent_summary"<?php if (!isset($_GET['summary'])) { echo " style=\"display: none;\""; } ?>>
          <form action="rfas.php" method="get" name="filterSummary" id="filterSummary">
            Display RFCs with the Summary containing <input type="text" name="summary" id="summary"<?php if (isset($_GET['summary'])) { echo " value=\"" . $_GET['summary'] . "\""; } ?> size="20" />
            <input type="submit" name="Submit" value="Submit" />
          </form>
        </div>    
        ...
        <div id="tabscontent_status"<?php if (!isset($_GET['status'])) { echo " style=\"display: none;\""; } ?>>
          <form action="rfas.php" method="get" name="filterStatus" id="filterStatus">
            Display <select name="status" id="status">
              ...
              <option value="Pending Approval"<?php if (isset($_GET['status']) && ($_GET['status'] == 'Pending Approval')) { echo " selected=\"selected\""; } ?>>Pending Approval</option>
              <option value="Pre-approved"<?php if (isset($_GET['status']) && ($_GET['status'] == 'Pre-approved')) { echo " selected=\"selected\""; } ?>>Pre-approved</option>
              <option value="Approved"<?php if (isset($_GET['status']) && ($_GET['status'] == 'Approved')) { echo " selected=\"selected\""; } ?>>Approved</option>
              <option value="Declined"<?php if (isset($_GET['status']) && ($_GET['status'] == 'Declined')) { echo " selected=\"selected\""; } ?>>Declined</option>
              <option value="Returned"<?php if (isset($_GET['status']) && ($_GET['status'] == 'Returned')) { echo " selected=\"selected\""; } ?>>Returned</option>
              <option value="Submitted for CAB Approval"<?php if (isset($_GET['status']) && ($_GET['status'] == 'Submitted for CAB Approval')) { echo " selected=\"selected\""; } ?>>Submitted for CAB Approval</option>
              <option value="Approved by CAB"<?php if (isset($_GET['status']) && ($_GET['status'] == 'Approved by CAB')) { echo " selected=\"selected\""; } ?>>Approved by CAB</option>
              <option value="Rejected by CAB"<?php if (isset($_GET['status']) && ($_GET['status'] == 'Rejected by CAB')) { echo " selected=\"selected\""; } ?>>Rejected by CAB</option>
              <option value="Returned by CAB"<?php if (isset($_GET['status']) && ($_GET['status'] == 'Returned by CAB')) { echo " selected=\"selected\""; } ?>>Returned by CAB</option>
              <option value="Completed"<?php if (isset($_GET['status']) && ($_GET['status'] == 'Completed')) { echo " selected=\"selected\""; } ?>>Completed</option>
              <option value="Resolved"<?php if (isset($_GET['status']) && ($_GET['status'] == 'Resolved')) { echo " selected=\"selected\""; } ?>>Resolved</option>
