...
FROM
(
SELECT procseq, datetime_event, period_proc FROM logmas_%%fecha%% WHERE `process`='%%process%%' AND type_proc='SS' %%where%% ORDER BY datetime_event DESC LIMIT 100
) AS A
LEFT JOIN ((
SELECT procseq_father, datetime_event, processed_rec, total_rec, files FROM logmas_%%fecha%% WHERE `process`='%%process%%' AND type_proc='ES' %%where%% ORDER BY datetime_event DESC LIMIT 100
) AS B) ON (A.procseq=B.procseq_father)
GROUP BY period
EOD;
//$where = '';
$period = "concat(date_format(str_to_date(A.period_proc, '%Y%m%d %h:%i'), '%d/%m %h'), ':00')";
...
if ($my_get['groupedby']) {
	switch ($my_get['groupedby']) {
		...
		case 2:
			...
			break;
		case 3:
			$period = "CONCAT(DATE_FORMAT(STR_TO_DATE(A.period_proc, '%Y%m%d %h:%i'), '%d/%m'), ' 00:00')";
			break;
	}
}
$labels = array();
